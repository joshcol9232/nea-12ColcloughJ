...
def test(singleInput=False):
    ...
    if singleInput:
        ...
        encSpd = 128/(time()-start)  # 128 BYTES in test message (due to padding.)

        start = time()
        dec = decryptFileName(key, enc)
        decSpd = 128/(time()-start)

        print(" Original:", test)
        print("Encrypted:", enc)
        print("Decrypted:", dec)

        print("Encrypt Speed:", encSpd/1000, "KB/s")
        print("Decrypt Speed:", decSpd/1000, "KB/s")

    else:
        def _makeList(wordNum):
            out = []
            for y in range(wordNum*16):
                out.append(chr(randint(32, 126))) # Expected input is either letters or punctuation.

            return " ".join(out)

        roundNum = 10000
        inp = _makeList(roundNum) # Create list beforehand to get accurate speed reading.
        print("inp done")
        start = time()
