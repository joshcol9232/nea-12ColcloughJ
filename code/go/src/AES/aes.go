...
func encryptFile(stringKey, f, w string) {
  ...
  var bufferSize int = 16384*getNumOfCores()
  ...
  for {
    ...
    n1, err := io.ReadAtLeast(a, buff, 1)
    _ = n1
    check(err)
    //fmt.Println(n1, len(buff))
    //fmt.Println(n1, buff, len(buff))
    //Check padding
    for {
      if len(buff) % 16 == 0 {
        break
      ...
      buff = append(buff, 0)
      fmt.Println(buff, "Adding zero.")
    }

    //fmt.Println("Got here", bufferSize/16)
    var encryptedBuff [][]byte
    if bufferSize <= 16 {
      var encrypted []byte
      encrypted = encrypt(buff, expandedKeys, 9)
      encryptedBuff = append(encryptedBuff, encrypted)
      //fmt.Println(encryptedBuff, "Encrypted buff")
    } else {

      for i := 0; i < bufferSize/16; i++ {
        var encrypted []byte
        encrypted = encrypt(buff[(16 * (i)):(16 * (i+1))], expandedKeys, 9)
        encryptedBuff = append(encryptedBuff, encrypted)
        //fmt.Println(encryptedBuff, "Encrypted buff")
      }
    }

    e, err := os.OpenFile(w, os.O_CREATE|os.O_RDWR|os.O_APPEND, 0644)
    check(err)
    if bufferSize <= 16 {
      e.Write(encryptedBuff[0])
    } else {
      for y := 0; y < bufferSize/16; y++ {
        e.Write(encryptedBuff[y])
        //fmt.Println(written, "Num of bytes written.")
      }
    }
...
func decryptFile(stringKey, f, w string) {
  ...
  for {
    ...
    if bufferSize <= 16 {
      var decrypted []byte
      decrypted = decrypt(buff, expandedKeys, 9)
      decryptedBuff = append(decryptedBuff, decrypted)
      //fmt.Println(decryptedBuff, "Encrypted buff")
    } else {

      for i := 0; i < bufferSize/16; i++ {
        var decrypted []byte
        decrypted = decrypt(buff[(16 * (i)):(16 * (i+1))], expandedKeys, 9)
        decryptedBuff = append(decryptedBuff, decrypted)
        //fmt.Println(decryptedBuff, "Encrypted buff")
      }
    }
...
func main(){
  // var exp [176]byte
  // var input = []byte {0x15, 0xb6, 0xef, 0xe9, 0x81, 0x61, 0xe1, 0x83, 0x53, 0x9d, 0x6e, 0xa3, 0xf4, 0x81, 0xdd, 0xe0}
  // key := padKey("abcdefghijklmno")
  // exp = expandKey(key, exp)
  // //fmt.Println(exp)
  // var returned []byte
  // returned = encrypt(input, exp, 9)
  // fmt.Println(returned, len(returned))
  // var ee []byte
  // ee = decrypt(returned, exp, 9)
  ...
  a := "/run/media/josh/USB/a.iso"

  encryptFile("abcdefghijklmno", f, w)
  decryptFile("abcdefghijklmno", w, a)
